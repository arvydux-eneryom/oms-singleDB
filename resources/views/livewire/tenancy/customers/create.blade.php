<div x-data="{ tab: 'general' }">
    ...
    @if ($errors->any())
        <div class="mb-6 rounded-lg border border-red-200 bg-red-50 px-6 py-4 text-red-800 shadow-sm">
                    ...
                    <div class="flex-1 flex flex-col gap-6">
                        <div class="grid grid-cols-6 gap-x-6 items-center">
                            <div class="col-span-5">
                                <flux:input wire:model.live.debounce.500ms="customer.company" :label="__('Company Name')" id="company"  badge="required" />
                            </div>
                            <div>
                        ...
                        <flux:input wire:model="customer.address" :label="__('Address')" id="autocomplete_address"  badge="required" />
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <flux:input wire:model="customer.country" :label="__('Country')" id="country_address" x-model="geo.country" />
                            <flux:input wire:model="customer.city" :label="__('City')" id="city_address" x-model="geo.city" />
                        </div>
                    </div>
                            ...
                            @foreach ($phones as $index => $phone)
                                <div class="relative bg-white border border-gray-200 rounded-lg shadow-sm p-6">
                                    <div class="flex justify-between items-center mb-4">
                                        ...
                                        @if(count($phones) > 1)
                                            <button type="button" wire:click="removePhone({{ $index }})" class="inline-flex items-center gap-2 px-3 py-1 text-sm font-medium text-gray-600 bg-gray-100 border border-gray-300 rounded-lg hover:bg-gray-200 hover:text-gray-900">🗑️ {{ __('Remove') }}</button>
                                        @endif
                                    </div>
                            ...
                            @foreach ($emails as $index => $email)
                                <div class="relative bg-white border border-gray-200 rounded-lg shadow-sm p-6">
                                    <div class="flex justify-between items-center mb-4">
                                        ...
                                        @if(count($emails) > 1)
                                            <button type="button" wire:click="removeEmail({{ $index }})" class="inline-flex items-center gap-2 px-3 py-1 text-sm font-medium text-gray-600 bg-gray-100 border border-gray-300 rounded-lg hover:bg-gray-200 hover:text-gray-900">🗑️ {{ __('Remove') }}</button>
                                        @endif
                                    </div>
                                    <flux:input
                    ...
                    @foreach ($contacts as $index => $contact)
                        <div class="relative bg-white border border-gray-200 rounded-lg shadow-sm p-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <flux:input wire:model="contacts.{{ $index }}.name" :label="__('Contact name')" id="contact-name-{{ $index }}" />
            ...
            document.querySelectorAll('input[id^="autocomplete_"]').forEach(input => {
                if (!input._autocompleteInitialized) {
                    ...
                    autocomplete.addListener('place_changed', function () {
                        ...
                        // Extract country, city, and post code from address components
                        let country = '';
                        let city = '';
                        let postcode = '';
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (input && !input._autocompleteInitialized) {
                ...
                autocomplete.addListener('place_changed', function () {
                    ...
                    // Trigger input events for Alpine/Livewire/x-model
                    ['country', 'city', 'postcode', 'latitude', 'longitude'].forEach(field => {
